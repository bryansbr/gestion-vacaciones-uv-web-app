document.addEventListener('DOMContentLoaded', function () {
  const btnCrear = document.getElementById('btn-crear-solicitud-secretaria');
        ...
        html: `
          <div class="flex flex-col gap-3 mt-4">
            ...
            >
              ...
              <p class="text-sm mt-1 opacity-90">Cree una solicitud de vacaciones para un funcionario bajo su Jefe Inmediato.</p>
            </button>
          </div>
        `,
            ...
            btnParaOtro.addEventListener('click', function() {
              ...
              if (!funcionariosData || funcionariosData.length === 0) {
                Swal.fire({
                  ...
                  text: 'No hay funcionarios bajo su Jefe Inmediato. Contacte al administrador.'
                });
                return;
              }
                ...
                if (result.isConfirmed && result.value) {
                  ...
                  if (!funcionarioSeleccionado.tiene_periodos) {
                    Swal.fire({
                      icon: 'error',
                      title: '¡Atención!',
                      html: `El funcionario <strong>${funcionarioSeleccionado.nombre} ${funcionarioSeleccionado.apellido}</strong> no tiene periodos vacacionales registrados. Contacte al administrador para que registre su(s) periodo(s) vacacional(es).`,
                      confirmButtonText: 'OK'
                    });
                    return;
                  }
                  ...
                  if (funcionarioSeleccionado && funcionarioSeleccionado.tiene_solicitud_activa) {
                    Swal.fire({
                      icon: 'info',
                      title: 'Solicitud en curso',
                      html: `El funcionario <strong>${funcionarioSeleccionado.nombre} ${funcionarioSeleccionado.apellido}</strong> ya tiene una solicitud de vacaciones en curso${funcionarioSeleccionado.solicitud_activa_codigo ? ` (${funcionarioSeleccionado.solicitud_activa_codigo})` : ''}.  Puede visualizarla en la opción "Listado de solicitudes".`,
                      confirmButtonText: 'OK'
                    });
                    return;
                  }
